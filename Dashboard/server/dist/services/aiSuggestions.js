...
exports.analyzeTaskPatterns = analyzeTaskPatterns;
 ...
 */
async function generateTaskSuggestions(limit = 5) {
    try {
        // Fetch current data
        const tasks = await (0, task_1.getAllTasks)();
        const events = await (0, event_1.getAllEvents)();
 ...
 */
function buildContextForAI(tasks, events, projects) {
    let context = '';
    // Projects
 ...
 */
function parseAISuggestions(response, projects) {
    const suggestions = [];
    ...
    for (const line of lines) {
        ...
        try {
            ...
            const priority = (priorityPart?.replace('PRIORITY:', '').trim().toLowerCase() || 'medium');
            ...
            if (projectName && projectName.toLowerCase() !== 'none') {
                const matchingProject = projects.find(p => p.name.toLowerCase().includes(projectName.toLowerCase()) ||
...
async function analyzeTaskPatterns() {
    ...
    const now = new Date();
    const overdueTasks = tasks.filter(t => !t.isCompleted &&
        t.dueDate &&
